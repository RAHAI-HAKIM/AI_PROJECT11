...
class Constraints:
    ...
    def SEPARATE_LECTURE_PRACTICE(self, state, count=True):
        ...
        for event_id, (roomid, slot) in state.items():
            ...
            day = slot // 6
            groups = (
                self.problem.section_to_group[event["target_id"]]
                if event["type_id"] == 1
    ...
    def MINIMIZE_WASTED_SEATS(self, state, weight):
        ...
        for event_id, (roomid, slot) in state.items():
            ...
            wasted += (room_data["capacity"] - event_data["headcount"]) / room_data[
                "capacity"
            ]  # take the percentage wasted
        ...
        for day, classes in days_active.items():
            ...
            for i in range(len(classes) - 1):
                ...
                if diff == 2 and (time2 == 2 or time2 == 5):
                    gaps += (
                        1  # penalize one session gaps that doesn't fall in lunch break
                    )
                elif diff == 3 and time2 != 4:
                    gaps += 1  # penalize two session gaps unless thay are in the break period 11:50-15:00
                else:
                    gaps += 1.5  # penalize more the 4 and 5 session gaps
        return gaps * weight
        ...
        for day, classes in days_active.items():
            ...
            for i in range(len(classes) - 1):
                ...
                if diff != 1:
                    continue
                if room1 == room2:
                    continue  # 0 cost for staying at the same room
                # load rooms data
                ...
                if r1_type == r2_type and r1_floor == r2_floor:
                    room_change_cost += 0.5  # same zone
                elif r1_type != r2_type and r1_floor != r2_floor:
                    room_change_cost += 2  # totaly different zone
                else:
                    room_change_cost += 1  # partially different zone
    ...
    def MINIMIZE_TEACHER_DAYS(self, schedule, weight):
        ...
        if num_days < num_sessions // 3:
            return 0
        return weight * (
            num_days + 1 - (num_sessions // 3)
        )  # the penalty increases with every additional day
