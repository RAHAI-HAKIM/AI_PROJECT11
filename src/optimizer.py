...
class Optimizer:
    ...
    def Random_Restart_Hill_Climbing(
        self, problem, objective=None, base_strategy="steepest", num_restarts=50
    ):
        """
        ...
        density_map = np.full((num_events, num_slots), 1.0 / num_slots)
        for every in range(num_restarts):
            current_state = {}
            for idx, event_id in enumerate(problem.events_by_id.keys()):
                slot_idx = np.random.choice(num_slots, p=density_map[idx])
                current_state[event_id] = problem.slots[slot_idx]
            result_state, _ = self.Hill_Climbing(
    ...
    def Tabu_Search(self, problem, objective=None, restarts=5, iters=300, tabu_size=20):
        ...
        for restart in range(restarts):
            ...
            state = (
                dict(problem.state)
                if restart == 0
                else dict(problem.generate_random_state())
            )
            best = dict(state)
